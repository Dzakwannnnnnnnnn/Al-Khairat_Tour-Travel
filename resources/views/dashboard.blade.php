            ...
            <div class="dashboard-card group animate-[fadeUp_1s_ease-out_0.1s_both]">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Total Jamaah</p>
                        <p class="text-xl md:text-3xl font-black text-slate-800 dark:text-slate-100 mt-1 md:mt-2 leading-none">{{ number_format($totalUsers) }}</p>
                    </div>
            ...
            <div class="dashboard-card group animate-[fadeUp_1s_ease-out_0.2s_both]">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Paket Layanan</p>
                        <p class="text-xl md:text-3xl font-black text-slate-800 dark:text-slate-100 mt-1 md:mt-2 leading-none">{{ $totalProducts }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card group animate-[fadeUp_1s_ease-out_0.3s_both]">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Running Tours</p>
                        <p class="text-xl md:text-3xl font-black text-slate-800 dark:text-slate-100 mt-1 md:mt-2 leading-none">{{ $activeProducts }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="dashboard-card group animate-[fadeUp_1s_ease-out_0.4s_both]">
                <div class="flex items-start justify-between">
                    <div>
                        ...
                        <p class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Total Valuasi</p>
                        <p class="text-lg md:text-2xl font-black text-slate-800 dark:text-slate-100 mt-1 md:mt-2 leading-none">Rp {{ number_format($totalRevenue / 1000000, 1, ',', '.') }}jt</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="glass-dashboard rounded-[2rem] p-5 md:p-8 animate-[fadeUp_1s_ease-out_0.5s_both]">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 px-2 gap-4">
                    <div>
                        <h2 class="text-lg md:text-xl font-bold text-slate-800 dark:text-slate-100">Log Registrasi Terbaru</h2>
                        <p class="text-[10px] md:text-xs text-slate-400 dark:text-slate-500 mt-1">Antrean jamaah yang baru bergabung sistem.</p>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="group flex items-center justify-between p-4 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800/50 border border-transparent hover:border-slate-100 dark:border-slate-800 transition-all duration-300">
                        <div class="flex items-center gap-4">
                            <div class="relative">
                                <img src="https://ui-avatars.com/api/?name={{ urlencode($user->name) }}&background=E07856&color=fff&size=80" 
                                     alt="{{ $user->name }}" class="w-12 h-12 rounded-xl object-cover border border-slate-100 dark:border-slate-800 group-hover:border-orange transition-colors">
                        ...
                        <div class="text-right">
                             <span class="inline-block px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest shadow-sm {{ $user->role === 'admin' ? 'bg-orange/10 text-orange border border-orange/20' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700' }}">
                                {{ $user->role }}
                            </span>

...
<div class="mb-8">
    <div class="flex items-center gap-4 mb-4">
        ...
        <div>
            <h1 class="text-3xl md:text-4xl font-bold text-charcoal">Edit Slideshow</h1>
            <p class="text-text/60 mt-1">Perbarui informasi slideshow</p>
        </div>
    </div>
</div>
...
<div class="max-w-2xl">
    <div class="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        <form method="POST" action="{{ route('slideshow.update', $slideshow) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="p-8 border-b border-slate-200">
                ...
                <!-- Current Image -->
                @if($slideshow->local_path || $slideshow->image_url)
                <div class="mb-6 p-4 border border-slate-200 rounded-lg bg-slate-50">
                    <p class="text-sm font-semibold text-text/60 mb-3">Gambar Saat Ini:</p>
                    <div class="max-w-xs">
                        @if($slideshow->local_path)
                            <img src="{{ asset('storage/' . $slideshow->local_path) }}" 
                                 alt="{{ $slideshow->title }}" class="w-full rounded-lg shadow-md border border-slate-200">
                        @else
                ...
                <div class="tab-content" id="tab-local">
                    <div class="mb-6">
                        <label class="block text-sm font-semibold text-charcoal mb-3">Pilih Gambar Baru</label>
                        <div class="relative">
                            ...
                            <label for="image" class="flex flex-col items-center justify-center w-full p-8 border-2 border-dashed border-slate-300 rounded-lg cursor-pointer hover:border-orange hover:bg-orange/5 transition-all duration-200">
                                ...
                                <p class="text-xs text-text/40 mt-3">Kosongkan jika tidak ingin mengubah | Format: JPG, PNG, GIF • Max 5MB</p>
                            </label>
                        </div>
                        @error('image')
                ...
                <div class="tab-content hidden" id="tab-url">
                    <div>
                        ...
                        <input type="url" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange focus:border-transparent @error('image_url') border-red-500 @enderror" 
                               name="image_url" placeholder="https://example.com/image.jpg" value="{{ old('image_url', $slideshow->image_url) }}">
                        @error('image_url')
                            <div class="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            </div>
                ...
                <div class="space-y-5">
                    <div>
                        ...
                        <input type="text" class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange focus:border-transparent @error('title') border-red-500 @enderror" 
                               name="title" placeholder="Contoh: Destinasi Indah" value="{{ old('title', $slideshow->title) }}">
                        @error('title')
                            <div class="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            </div>
                    ...
                    <div>
                        ...
                        <textarea class="w-full px-4 py-3 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange focus:border-transparent resize-none @error('description') border-red-500 @enderror" 
                                  name="description" rows="4" placeholder="Deskripsi singkat tentang slideshow ini...">{{ old('description', $slideshow->description) }}</textarea>
                        @error('description')
                            <div class="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg">
                                <p class="text-sm text-red-600">{{ $message }}</p>
                            </div>
                    ...
                    <div class="flex items-center gap-3 p-4 bg-slate-50 rounded-lg border border-slate-200">
                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $slideshow->is_active) ? 'checked' : '' }} class="w-5 h-5 text-orange rounded focus:ring-2 focus:ring-orange">

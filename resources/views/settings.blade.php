...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center p-6 bg-white rounded-lg shadow-sm">
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Pengaturan Variabel (Key-Value)</h2>
                <p class="text-sm text-gray-500 mt-1">Konfigurasi dinamis untuk kontak, sosmed, rekening, dan info esensial.</p>
            </div>
        </div>
        <button onclick="document.getElementById('addSettingModal').classList.remove('hidden')" class="mt-4 sm:mt-0 flex items-center space-x-2 bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition shadow-sm font-medium">
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 align-top">
                            <p class="font-mono text-sm font-semibold text-indigo-600">{{ $setting->key }}</p>
                        </td>
                        ...
                        <td class="px-6 py-4 align-top whitespace-nowrap">
                            ...
                            <form method="POST" action="{{ route('settings.destroy', $setting) }}" class="inline" onsubmit="return confirm('Sangat berbahaya menghapus pengaturan sistem. Lanjutkan?')">
                                @csrf
                                @method('DELETE')
...
<div id="editSettingModal" class="hidden fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
        ...
        <form id="editSettingForm" method="POST" class="space-y-4">
            ...
            <div class="flex space-x-3 pt-4 border-t border-gray-100">
                <button type="button" onclick="document.getElementById('editSettingModal').classList.add('hidden')" class="flex-1 px-4 py-2 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition">Batal</button>
